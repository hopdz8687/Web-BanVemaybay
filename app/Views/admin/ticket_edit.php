...
<div class="container-lg py-4 page-wrap">
  ...
  <form method="post" action="<?=base_url('/admin/flights/tickets/edit')?>?ticket_id=<?=$ticket['id']?><?=!empty($q) ? '&q=' . urlencode($q) : ''?>" class="row g-3">
    <div class="col-12 col-md-6">
      <label class="form-label">Mã vé</label>
      <input name="ma_ve" class="form-control" required value="<?=htmlspecialchars($maVe)?>">
    </div>
    ...
    <div class="col-12 col-md-6">
      ...
      <input type="number" step="0.01" min="0" name="gia" class="form-control" required value="<?=htmlspecialchars((string)($gia ?? ''))?>">
    </div>
