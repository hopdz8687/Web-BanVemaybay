  ...
  <div class="row justify-content-center">
    <div class="col-md-8">
      <div class="card shadow">
          ...
          <h4 class="text-dark mb-0"><i class="bi bi-pencil-square"></i> Sửa Chuyến Bay</h4>
        </div>
        <div class="card-body p-4">
          ...
          <form method="post" action="<?=base_url('/admin/flights/edit')?>?id=<?=$flight['id']?>">
            <div class="mb-3">
              <label class="form-label">Số hiệu chuyến bay</label>
              <input type="text" name="so_hieu" class="form-control form-control-lg" value="<?=htmlspecialchars($flight['so_hieu'])?>" readonly>
            </div>
            <div class="row mb-3">
              <div class="col-md-6">
                ...
                <select id="airlineSelect" class="form-select form-select-lg" <?=empty($planes) ? 'disabled' : ''?>>
                  ...
                  <option value="Khác" <?=($selectedAirline ?? '')==='Khác'?'selected':''?>>Khác</option>
                </select>
              </div>
              <div class="col-md-6">
                ...
                <select id="planeSelect" name="may_bay_id" class="form-select form-select-lg" <?=empty($planes) ? 'disabled' : ''?>>
                  ...
                  <?php if (!empty($planes)): foreach ($planes as $p): ?>
                    <option value="<?=$p['id']?>" data-airline="<?=htmlspecialchars($p['hang_may_bay'] ?? 'Khác')?>" <?=((string)($selectedPlaneId ?? '') === (string)$p['id']) ? 'selected' : ''?>>
            ...
            <div class="row mb-3">
              <div class="col-md-6">
                ...
                <input type="text" name="noi_di" class="form-control form-control-lg" value="<?=htmlspecialchars($flight['noi_di'])?>" required>
              </div>
              <div class="col-md-6">
                <label class="form-label">Điểm đến</label>
                <input type="text" name="noi_den" class="form-control form-control-lg" value="<?=htmlspecialchars($flight['noi_den'])?>" required>
              </div>
            </div>
            <div class="row mb-3">
              <div class="col-md-6">
                <label class="form-label">Thời gian khởi hành</label>
                <input type="datetime-local" name="gio_khoi_hanh" class="form-control form-control-lg" value="<?=htmlspecialchars($gioKhoiHanhLocal ?? '')?>" required>
              </div>
              <div class="col-md-6">
                <label class="form-label">Thời gian đến</label>
                <input type="datetime-local" name="gio_ha_canh" class="form-control form-control-lg" value="<?=htmlspecialchars($gioHaCanhLocal ?? '')?>" required>
              </div>
            </div>
            <div class="row mb-3">
              <div class="col-md-4">
                <label class="form-label">Giá vé thường (VND)</label>
                <input type="number" name="gia_thuong" class="form-control form-control-lg" value="<?=htmlspecialchars($flight['gia_thuong'])?>" required>
              </div>
              <div class="col-md-4">
                <label class="form-label">Giá vé thương gia (VND)</label>
                <input type="number" name="gia_thuong_gia" class="form-control form-control-lg" value="<?=htmlspecialchars($flight['gia_thuong_gia'])?>" required>
              </div>
              <div class="col-md-4">
                <label class="form-label">Số ghế</label>
                <input type="number" name="ghe_con" class="form-control form-control-lg" value="<?=htmlspecialchars($flight['ghe_con'])?>" required>
              </div>
            </div>
...
<script>
  (function(){
    ...
    function filterPlanes(keepSelected){
      var airline = airlineSelect.value;
      var selected = keepSelected ? planeSelect.value : '';
      ...
      if (keepSelected && selected) {
        planeSelect.value = selected;
      }
    }

    airlineSelect.addEventListener('change', function(){ filterPlanes(false); });
    filterPlanes(true);
  })();
</script>
</body>
